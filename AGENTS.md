...
Use `cargo add <crate>@<version>` (with `--features` as needed) to introduce new dependencies, which keeps `Cargo.toml` and `Cargo.lock` synchronized. Do not edit `Cargo.toml` manually for additions that the `cargo add` workflow supports; rely on direct edits only for metadata tweaks or settings unavailable through the command.
