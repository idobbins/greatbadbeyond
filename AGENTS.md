...
- `src/main.cpp` owns the application entry point; it spins up GLFW, builds a `VulkanConfig`, drives the event loop, and tears everything down.
...
- `src/runtime.h` implements inline logging helpers and the `Assert` utility that all translation units rely on.
...
- Logging flows through `LogInfo`, `LogWarn`, and `LogError` in `runtime.h`; reuse them for consistent prefixes.
...
Struct types | TitleCase | `struct VulkanConfig`
